...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x_input, mask, cache=None):
        ...
        ##
        x_acc_emb = self.accent_emb.repeat(1,8,1).to('cuda:0')
        #print("x_acc_emb.shape: ", x_acc_emb.shape)
        x_q = x_q.permute(1, 0, 2).to('cuda:0')
        ##

        # print(x_acc_emb.shape)
        #self.embedding_projection = torch.nn.Sequential(x_acc_emb, torch.nn.Softsign())

        def _integrate_with_lang_embed(self, hs, lang_embeddings):
            # project embedding into smaller space
            lang_embeddings_projected = self.embedding_projection(lang_embeddings)
            # concat hidden states with spk embeds and then apply projection
            lang_embeddings_expanded = F.normalize(lang_embeddings_projected).unsqueeze(1).expand(-1, hs.size(1), -1)
            hs = self.hs_emb_projection(torch.cat([hs, lang_embeddings_expanded], dim=-1))
            return hs
        ...
        if pos_emb is not None:
            ...
            print(x_q.shape)
            print("key and value = x_acc_emb.shape") # [8, 12, 384]
            print(x_acc_emb.shape)
            print("pos_emb.shape")   # [1, 265, 384] pos_emb: :math:`(N, 2*L-1, E)` or :math:`(1, 2*L-1, E)` where L is the target sequence length (query), N is the batch size, E is the embedding dimension.
            print(pos_emb.shape)
            print("x.get_device", x.get_device())
            mask = mask.permute(0,2,1)
            print("mask.shape")      # [8, 133, 1]
            print(mask.shape)
            #x_att = self.self_attn(x_q, x_acc_emb, x_acc_emb, pos_emb, mask)
            rel_pos_multihead_attn = RelPositionMultiHeadAttention(384, 4).to('cuda:0')
            x_att = rel_pos_multihead_attn(x_q, x_acc_emb, x_acc_emb, pos_emb)
            #x_att = RelPositionMultiHeadAttention(x_q, x_acc_emb, x_acc_emb) # braucht anderen shape: `(L, N, E)` where L is the target sequence length, N is the batch size, E is the embedding dimension.
            #x_att = RelPositionMultiHeadedAttention(x_q, x_acc_emb, x_acc_emb)
